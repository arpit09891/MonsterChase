...
void LuaTestClass::LuaTest(uint8_t * pFileContents, size_t sizeFile)
{
	...
	//REad in File
	//its gonna be in main 
	/*size_t sizeFile = 0;
	uint8_t * pFileContents = Engine::LoadFile("data\\Player.Lua",sizeFile);*/
	...
	if (pFileContents  && sizeFile)
	{
		...
		assert(result == 0,"result is zero");
		...
		assert(result == LUA_TTABLE, "result is not LUA_TAble");
		...
		assert(type == LUA_TSTRING,"type is not string");
		...
		type = lua_gettable(pLuaState, -2);
		assert(type == LUA_TTABLE,"type is not lua table");
		...
		assert((type == LUA_TNIL) || (type == LUA_TTABLE), "type is not lua table or null value");

		if (type == LUA_TTABLE)
		{
			lua_pushstring(pLuaState, "mass");
			type = lua_gettable(pLuaState, -2);
			assert(type == LUA_TNUMBER, "type is not number");
		...
		assert(stack_items == 0, "stack items is not 0");
		void * test = reinterpret_cast<void*>(const_cast<char *>(pName));
		free(test);
...
int LuaTestClass::GetLuaTableEntries(lua_State * i_pState, int i_index)
{
	assert(i_pState,"i_pState is null");
	assert(lua_gettop(i_pState) >= -i_index,"i_pState has issues");
	assert(lua_type(i_pState, i_index) == LUA_TTABLE,"Lua Table is not equal to lua type");

	lua_len(i_pState, i_index);
	assert(lua_type(i_pState, -1) == LUA_TNUMBER,"Lua number is not equal to lua type number");

	lua_Integer table_entries = lua_tointeger(i_pState, -1);
	lua_pop(i_pState, 1);

	assert(table_entries <= INT_MAX,"table entries are greater than max enteries");

	return static_cast<int>(table_entries);
}

size_t LuaTestClass::ReadIntArray(lua_State * i_pState, int i_index, int * o_pInts, size_t i_numInts)
{
	assert(i_pState, "i_pState is null");
	assert(lua_gettop(i_pState) >= -i_index, "i_pState has issues");
	assert(lua_type(i_pState, i_index) == LUA_TTABLE, "Lua Table is not equal to lua type");
	...
	while (lua_next(i_pState, i_index - 1) != 0)
	{
		if (lua_type(i_pState, -1) == LUA_TNUMBER)
		{
			...
			assert(lValue <= INT_MAX,"lValue exceeds max value");
...
size_t LuaTestClass::ReadFloatArray(lua_State * i_pState, int i_index, float * o_pFloats, size_t i_numFloats)
{
	assert(i_pState, "i_pState is null");
	assert(lua_gettop(i_pState) >= -i_index, "i_pState has issues");
	assert(lua_type(i_pState, i_index) == LUA_TTABLE, "Lua Table is not equal to lua type");
...
size_t LuaTestClass::GetFloatArray(lua_State * i_pState, int i_indexTable, const char * i_pArrayKey, float * o_pFloats, size_t i_numFloats)
{
	assert(i_pArrayKey,"i_pArrayKey is null");
...
Point2D LuaTestClass::GetVector3(lua_State * i_pState, int i_index)
{
	assert(i_pState,"i_pState is null");
	assert(lua_gettop(i_pState) >= -i_index, "i_pState has issues");
	assert(lua_type(i_pState, i_index) == LUA_TTABLE, "Lua Table is not equal to lua type");
